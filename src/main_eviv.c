...
//window 5 = POKEMON EV-IV          +PAGE (A)(B)EXIT
...
const u8 gText_eviv_Buttons[] = _("{DPAD_UPDOWN}Page {A_BUTTON}{B_BUTTON}Exit");
...
static void PrintWindow0(struct Pokemon *mon)
{
    ...
    AddTextPrinterParameterized3(WIN_POKEMON_NAME, 2, 0x44, 2, sGrayTextColor, 0, gText_BsEvIv);

    GetMonNickname(mon, gStringVar4);
    AddTextPrinterParameterized3(WIN_POKEMON_NAME, 2, 0x92, 2, sGrayTextColor, 0, gStringVar4);
}
...
static void PrintWindow_HiddenPower(u8 isEgg)
{
    ...
    AddTextPrinterParameterized3(WIN_HIDDEN_POWER, 0, 8, 3, sBlackTextColor, 0, gStringVar4);
    ...
    if (!isEgg)
    {
        ...
        StringAppend(gStringVar4, gStringVar1);
        AddTextPrinterParameterized3(WIN_HIDDEN_POWER, 0, 8, 16, sBlackTextColor, 0, gStringVar4);
        BlitMoveInfoIcon(WIN_TYPE, type + 1, 2, 1);
    }
    else
        AddTextPrinterParameterized3(WIN_HIDDEN_POWER, 0, 36, 16, sBlackTextColor, 0, gText_q);
}
